...
export class WompiService implements PaymentServiceInterface {
  ...
  private generateIntegritySignature(
    ...
    // Wompi signature format: reference + amount_in_cents + currency + integrity_secret
  ...
  async createPaymentSource(cardData: CardData): Promise<PaymentSource> {
    try {
      const response = await this.httpClient.post('/payment_sources', {
        type: 'CARD',
        token: cardData,
      });
  ...
  async createTransaction(
    ...
    try {
      ...
      this.logger.log(`Creating Wompi transaction with reference: ${transactionData.reference}`);
      ...
      this.logger.log(`Wompi transaction created: ${data.id}, status: ${data.status}`);
      ...
      if (axiosError.code === 'ECONNREFUSED') {
        throw new Error('Cannot connect to Wompi API. Check network connection.');
      }
