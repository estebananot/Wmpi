...
export class TransactionsController {
  ...
  async getTransaction(@Param('id') id: string): Promise<{ data: TransactionResponseDto }> {
  ...
  @ApiResponse({ status: 404, description: 'Transaction not found' })
  @ApiResponse({ status: 409, description: 'Transaction already processed or insufficient stock' })
  async processPayment(
    ...
    if (result.isFailure) {
      ...
      if (message.includes('cannot be processed') || message.includes('Insufficient stock')) {
        throw new HttpException(message, HttpStatus.CONFLICT);
      }
      throw new HttpException(message, HttpStatus.BAD_REQUEST);
    }
