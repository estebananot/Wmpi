  ...
  async execute(id: string): Promise<Result<TransactionResponseDto, Error>> {
    try {
      ...
      if (transaction.status === 'PENDING' && transaction.wompiTransactionId) {
        try {
          const wompiStatus = await this.paymentService.getTransaction(transaction.wompiTransactionId);
          
          if (wompiStatus.status !== 'PENDING') {
            this.logger.log(`Updating transaction ${id} status from PENDING to ${wompiStatus.status}`);
