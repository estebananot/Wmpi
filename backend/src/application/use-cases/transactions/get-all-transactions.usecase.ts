...
export class GetAllTransactionsUseCase {
  ...
  async execute(): Promise<Result<TransactionResponseDto[], Error>> {
    try {
      ...
      const response: TransactionResponseDto[] = transactions.map(transaction => ({
        id: transaction.id,
      ...
      return Result.fail(error instanceof Error ? error : new Error('Failed to get transactions'));
    }
  }
}
