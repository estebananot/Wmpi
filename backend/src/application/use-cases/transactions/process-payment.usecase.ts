...
import { DEFAULT_CUSTOMER } from '../../../domain/entities/customer.entity';
...
export class ProcessPaymentUseCase {
  ...
  async execute(
    ...
    try {
      ...
      this.logger.log(`Transaction found: ${transaction.transactionNumber}, status: ${transaction.status}`);

      // Validate that transaction is in PENDING state
      if (!transaction.canBeProcessed()) {
        this.logger.error(`Transaction cannot be processed. Current status: ${transaction.status}`);
        return Result.fail<TransactionResponseDto, PaymentError>(
          new PaymentError(
      ...
      if (product.stock < transaction.quantity) {
        this.logger.error(`Insufficient stock. Available: ${product.stock}, Requested: ${transaction.quantity}`);
        return Result.fail<TransactionResponseDto, PaymentError>(
          new PaymentError('Insufficient stock to complete payment'),
        );
      }

      this.logger.log(`Creating Wompi transaction for amount: ${transaction.totalAmount} COP`);
      ...
      const wompiResponse = await this.paymentService.createTransaction({
        ...
        customer_data: {
          phone_number: DEFAULT_CUSTOMER.phone || '+573000000000',
          full_name: DEFAULT_CUSTOMER.name || 'Usuario de Prueba',
        },
      });

      this.logger.log(`Wompi response: id=${wompiResponse.id}, status=${wompiResponse.status}`);
      ...
      this.logger.error(`Payment processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return Result.fail<TransactionResponseDto, PaymentError>(
        new PaymentError(
